...
Download the appropriate `cerbos_aws_lambda_func` tarball from the link:{app-github-releases-page}[releases page]. It contains a specially-built binary named `bootstrap` that can be deployed as either a Zip package or a container image.

Deploying it as a zip package requires creating a zip file with the `bootstrap` binary at the root. See link:https://docs.aws.amazon.com/lambda/latest/dg/golang-package.html[Deploy Go Lambda functions with .zip file archives] for more information.
...
Download the appropriate `cerbos_aws_lamba_ext` tarball from the link:{app-github-releases-page}[releases page]. It contains a specially-built `cerbosext` binary that can be deployed as a Zip layer.
