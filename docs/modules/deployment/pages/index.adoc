...
| Requires rolling update of nodes

| Policy propagation
| Single instance to update
...
| Best for
| Small clusters, simple setups, non-Kubernetes environments
| Latency-sensitive applications, strict isolation requirements
| Large clusters with many pods per node
|===
...
The *service* model is the simplest to operate and is a good starting point. It works well for non-Kubernetes environments and smaller clusters where a central service won't become a bottleneck.

The *DaemonSet* model is efficient for large clusters where running a sidecar per pod would be wasteful. It provides node-local performance without the overhead of one Cerbos instance per application pod.
