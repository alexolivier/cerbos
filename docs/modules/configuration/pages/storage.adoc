...
Connects the PDP to a Cerbos Hub xref:cerbos-hub:ROOT:deployments.adoc[Deployment]. A Cerbos Hub Deployment tracks a connected Git repository or a xref:cerbos-hub:ROOT:policy-stores.adoc[managed Cerbos Hub store] and automatically compiles, tests and pushes an optimized policy bundle to the PDP whenever a policy change is committed.

If you are new to Cerbos Hub, follow the xref:cerbos-hub:ROOT:getting-started.adoc[getting started guide]. For more information about configuring a PDP to connect to Cerbos Hub, refer to the xref:cerbos-hub:ROOT:deployments.adoc[Deployments documentation].
...
[TIP]
====
Cerbos Hub builds, signs, and distributes policy bundles to connected PDPs without requiring local storage configuration. See https://docs.cerbos.dev/cerbos-hub/[the Cerbos Hub documentation] for details.
====
