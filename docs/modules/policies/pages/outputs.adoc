...
Outputs are supported on rules in xref:resource_policies.adoc[resource policies],
xref:principal_policies.adoc[principal policies], and xref:role_policies.adoc[role policies].
The example below uses a resource policy; the `output` field has the same shape on all three.
...
Output values are free-form, with one exception. When the value is a map, the key `cerbos.accessRequest` is reserved: Cerbos Hub reads it as an escalation marker, which turns the deny into a xref:requestable_denies.adoc[requestable deny]. Treat the `cerbos.` prefix as reserved for Cerbos and do not use it for your own keys.
