...
[.lead]
Conditions are CEL expressions evaluated at request time against principal, resource, and request attributes. A rule with a condition applies only when the expression evaluates to true.
...
[TIP]
====
For patterns that translate condition logic into database queries, see https://www.cerbos.dev/blog/filtering-database-results-with-cerbos-query-plans[Filtering database results with Cerbos query plans]. For implementation review, https://www.cerbos.dev/workshop?utm_campaign=brand_cerbos&utm_source=documentation&utm_medium=text[speak to a Cerbos engineer].
====
