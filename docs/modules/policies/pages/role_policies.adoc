...
[.lead]
Role policies define the set of actions a role is permitted to perform on each resource. The allowable-actions list is exhaustive: any action not listed is denied for that role.
...
[TIP]
====
For a walkthrough of translating business access requirements into Cerbos policies, see https://www.cerbos.dev/blog/mapping-business-requirements-to-authorization-policy[Mapping business requirements to authorization policy].
====
